...
import Style from "./css/NftCard.css"
      ...
      <Grid.Container gap={4}>
        {nfts.map((nft, i) => {
            return (
              <Grid >
                <a>
                  <Card isHoverable key={i} css={{ mw: "270px", marginRight: '$1', boxShadow:'0px 2px 12px #000000', width: "2000px", float:"left" }} variant="bordered">
                    <Card.Image src={nft.img} />
                    <div className={Style.Nft_description} css={{  float:"right" }}>
                    <Card.Body md css={{background:"$white"}}>
                    <Text css={{color:'$black'}} h2>{nft.name}</Text>
                    <Text h3 css={{color:'$black'}}>NFT ID: {nft.tokenId}</Text>
                    <Text css={{color:'$black'}}>{nft.desc}</Text>
                    </Card.Body>
                    </div>
                  </Card>
                </a>
